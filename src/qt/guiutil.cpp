...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a snarkcoin address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no snarkcoin: URI
...
{
    // Convert snarkcoin:// to snarkcoin:
    //
    //    Cannot handle this later, because snarkcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open snarkcoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "snarkcoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for snarkcoin.lnk
...
{
    return GetAutostartDir() / "snarkcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a snarkcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=snarkcoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the snarkcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add snarkcoin app to startup item list
