...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizesnarkcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizesnarkcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizesnarkcoinAmount = strAnonymizesnarkcoinAmount.remove(strAnonymizesnarkcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizesnarkcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizesnarkcoinAmount * COIN) nMaxToAnonymize = nAnonymizesnarkcoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizesnarkcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizesnarkcoinAmount));
        strAnonymizesnarkcoinAmount = strAnonymizesnarkcoinAmount.remove(strAnonymizesnarkcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizesnarkcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizesnarkcoinAmount == 0) {
